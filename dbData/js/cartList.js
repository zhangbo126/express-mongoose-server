db.getCollection("cartList").insert( {
    __v: NumberInt("0"),
    _id: ObjectId("6187c4154e2ff417ed9ee4d3"),
    brandId: "61751f3fb5638ec5907e7b92",
    brandName: "下关沱茶",
    categoryId: "6175162f3cdcb0a414268230",
    categoryName: "绿茶",
    designSketch: [
        "http://192.168.3.18:99/upload/2021/10/24/202110242031_3280.1318062869655.jpg",
        "http://192.168.3.18:99/upload/2021/10/24/202110242031_3917.1487100290083.jpg",
        "http://192.168.3.18:99/upload/2021/10/24/202110242031_6767.082933858719.jpg"
    ],
    goodsName: "武夷大红袍",
    goodsNo: "WYDHP-001",
    goodsType: NumberInt("2"),
    mixDetail: null,
    num: NumberInt("2"),
    placeOrigin: "四川",
    price: NumberInt("2342"),
    salesVolume: null,
    skuId: "617552327c0bb6cfacb2488f",
    skuName: "武夷大红袍特价细茶",
    specValue1: "大红袍粗茶",
    specValue2: "武夷大红袍打折",
    status: NumberInt("1"),
    userId: "6181594c834e15ff4892024f",
    weight: NumberInt("2342")
} );
db.getCollection("cartList").insert( {
    __v: NumberInt("0"),
    _id: ObjectId("61a2fbde724c4b27b17e8c63"),
    brandId: "61a2366c1b10f1139d84e507",
    brandName: "福茗源",
    categoryId: "61a23b711b10f1139d84e511",
    categoryName: "俊旅-红茶",
    designSketch: [
        "http://localhost:99/upload/2021/11/27/202111272247_6745.647025132686.jpg",
        "http://localhost:99/upload/2021/11/27/202111272247_4619.585996469211.jpg"
    ],
    goodsName: "福茗源秒杀系列",
    goodsNo: "FMY-001",
    goodsType: NumberInt("2"),
    mixDetail: "<p><img src=\"http://localhost:99/upload/2021/11/27/1464607509917601792.jpg\" title=\"\" alt=\"5af2e514N5931048c.jpg\"/></p><p><img src=\"http://localhost:99/upload/2021/11/27/1464607539554553856.jpg\" title=\"\" alt=\"c48471bbedce7c0a.jpg\"/></p>",
    num: NumberInt("1"),
    placeOrigin: "江苏",
    price: NumberInt("2342"),
    salesVolume: NumberInt("54"),
    skuId: "61a244fb2ee28a1f3c799319",
    skuName: "福茗源-细茶（热销\t）\t）",
    specValue1: "特价",
    specValue2: "热销",
    status: NumberInt("1"),
    userId: "619a467d36568547d078b7a2",
    weight: NumberInt("234")
} );
db.getCollection("cartList").insert( {
    __v: NumberInt("0"),
    _id: ObjectId("61a2fbe7724c4b27b17e8c64"),
    brandId: "61a2367c1b10f1139d84e508",
    brandName: "同仁堂",
    categoryId: "61a2467d2ee28a1f3c79931d",
    categoryName: "同仁堂养生茶",
    designSketch: [
        "http://localhost:99/upload/2021/11/27/202111272258_3361.4240506109304.jpg",
        "http://localhost:99/upload/2021/11/27/202111272258_9451.99574027899.jpg",
        "http://localhost:99/upload/2021/11/27/202111272258_3459.3379473286423.jpg"
    ],
    goodsName: "同仁堂养生茶",
    goodsNo: "TRT-001",
    goodsType: NumberInt("1"),
    mixDetail: null,
    num: NumberInt("1"),
    placeOrigin: "南京",
    price: NumberInt("432"),
    salesVolume: NumberInt("26"),
    skuId: "61a248172ee28a1f3c79931f",
    skuName: "同仁堂粗茶（特惠）",
    specValue1: "特价",
    specValue2: "特惠",
    status: NumberInt("1"),
    userId: "619a467d36568547d078b7a2",
    weight: NumberInt("532")
} );
db.getCollection("cartList").insert( {
    __v: NumberInt("0"),
    _id: ObjectId("61a2fbf1724c4b27b17e8c65"),
    brandId: "61a236851b10f1139d84e509",
    brandName: "花果茶",
    categoryId: "61a23b4a1b10f1139d84e510",
    categoryName: "乐品乐-绿茶",
    designSketch: [
        "http://localhost:99/upload/2021/11/27/202111272328_784.4556600347885.jpg"
    ],
    goodsName: "乐品乐花果茶\t",
    goodsNo: "LPL-002",
    goodsType: NumberInt("1"),
    mixDetail: null,
    num: NumberInt("1"),
    placeOrigin: "山东",
    price: NumberInt("432"),
    salesVolume: NumberInt("63"),
    skuId: "61a24eb4724c4b27b17e8c59",
    skuName: "乐品乐花果茶细茶（特价）",
    specValue1: "特价",
    specValue2: "秒杀",
    status: NumberInt("1"),
    userId: "619a467d36568547d078b7a2",
    weight: NumberInt("33")
} );
